...
Buffer 未满时返回 `None`，`infer/runtime.py` 会报告 `memory not ready`，不会自动补零。
...
| key | shape | 说明 |
|---|---|---|
| `memory_image_backbone_feat` | `[B,16,V,384]` | DINO global 历史 |
| `memory_state` | `[B,64,Ds]` | 归一化的状态历史 |
| `memory_visual_offsets` | `[16]` | `[-64,-60,...,-8,-4]` |

offsets 不扩展成 32 或 48。V 个视角作为 batch 独立处理，共享同一组
16 个时间 offsets。

## 5. 每视角独立时间建模

```text
feature_window                    [B,16,V,384]
permute(0,2,1,3)                 [B,V,16,384]
reshape                           [B*V,16,384]
shared DINO projection           [B*V,16,256]
shared Temporal Transformer      [B*V,16,256]
mean over 16 time outputs        [B*V,256]
reshape                           [B,V,256]
...
Temporal Transformer 也只有一套参数。它处理 `B*V` 个长度为 16 的序列，
而不是一个长度为 `16*V` 的序列。这里不会再添加第 17 个 temporal CLS；
Transformer 输出的 16 个时间 token 直接做 valid-aware mean pooling。
...
# Buffer 满 16 个采样时刻后才能调用 Memory Policy。
...
| 功能 | 文件 |
|---|---|
| 异步 DINO 和 16 帧 Buffer | `tools/async_dino_buffer.py` |
